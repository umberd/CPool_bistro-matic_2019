...
char *my_strcpy_spec(char *dest, char const *src, char *base)
{
    int i = 0;
...
char *nb_comp_equalize(char *result, int len_result, char *base)
{
    my_revstr(result);
    result = malloc_digit(result, len_result);
    for (int i = 0; i < len_result; i++)
        if (base_i(result[i], base) == -1)
            result[i] = base[0];
...
char *inf_sub_two(char *nb1, char *nb2, int *is_neg, char *base)
{
    ...
    if (len1 > len2)
        nb2_cpy = nb_comp_equalize(nb2_cpy, len1, base);
...
char *inf_sub(char *str1, char *str2, char *base, char *sp)
{
    ...
    if (str1[0] == sp[3] && str2[0] != sp[3]) {
        is_neg = 1;
        result = inf_add(&str1[1], &str2[0], base, sp);
    }
    if (str1[0] != sp[3] && str2[0] == sp[3])
